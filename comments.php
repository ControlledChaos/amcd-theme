...
<?php comment_form( AMCD_Theme_Comments_Form::args() );

if ( have_comments() ) :

	echo '<h3 class="comments-title">' . AMCD_Theme_Comments_Heading::heading() . '</h3>';

	if ( ! comments_open() && post_type_supports( get_post_type(), 'comments' ) ) {
		echo AMCD_Theme_Comments_Status::closed();
	} ?>
...
<?php else :
	...
	if ( comments_open() && post_type_supports( get_post_type(), 'comments' ) ) {
		echo AMCD_Theme_Comments_Status::none();
